...
const PropertiesView = ({ properties, addProperty, updateProperty, deleteProperty }) => {
  ...
  const [formData, setFormData] = useState({
    title: '', location: '', area: '', price: '', category: 'Residential', badge: 'NEW', features: ''
  });

  const handleEdit = (index) => {
    setEditingIndex(index);
    const p = properties[index];
    setFormData({ ...p, features: p.features.join(', ') });
    setIsAdding(true);
  };
  ...
  const handleSubmit = (e) => {
    ...
    setFormData({ title: '', location: '', area: '', price: '', category: 'Residential', badge: 'NEW', features: '' });
  };
  ...
  return (
    <div className="space-y-10">
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[100] flex items-center justify-center p-8">
          <div className="bg-[#0a0a0a] border border-white/10 rounded-[3rem] p-12 w-full max-w-3xl max-h-[90vh] overflow-y-auto relative shadow-[0_0_100px_rgba(0,0,0,0.8)]">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-10">
              ...
              <div className="col-span-2">
                <label className="block text-[11px] font-black text-gray-500 uppercase tracking-[0.3em] mb-3 opacity-60">Features (Comma Delimited)</label>
                <input required value={formData.features} onChange={e => setFormData({...formData, features: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-2xl px-8 py-5 focus:border-gold/50 outline-none text-white font-bold transition-all" placeholder="Infinity Pool, Smart Home, Gated Community" />
              </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
        {properties.map((p, i) => (
          <div key={i} className="bg-[#0a0a0a]/80 backdrop-blur-md border border-white/5 rounded-[3rem] p-10 flex flex-col md:flex-row justify-between items-start md:items-center hover:border-gold/40 transition-all group relative overflow-hidden shadow-2xl">
            ...
            <div className="flex gap-8 items-center relative">
              <div className="w-24 h-24 bg-black/40 rounded-3xl flex items-center justify-center text-4xl group-hover:scale-110 group-hover:rotate-3 transition-transform duration-700 border border-white/10 shadow-inner">
                {p.category === 'Commercial' ? '🏢' : '🏡'}
              </div>
              <div>
                <div className="flex items-center gap-4 mb-2">
                  <h4 className="font-bold text-2xl tracking-tight text-white/90">{p.title}</h4>
                  <span className={`text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-widest border ${i % 3 === 0 ? 'bg-green-500/10 text-green-400 border-green-500/20' : 'bg-gold/10 text-gold border-gold/20'}`}>
                    {i % 3 === 0 ? 'Live' : 'Published'}
                  </span>
                </div>
                <div className="flex flex-wrap gap-x-8 gap-y-3 text-[11px] text-gray-500 font-black uppercase tracking-[0.2em]">
                  ...
                  <span className="flex items-center gap-2">📏 {p.area} SQ.FT</span>
            ...
            <div className="hidden 2xl:flex items-center gap-10 ml-auto mr-16 py-4 px-10 bg-black/40 rounded-[2rem] border border-white/10 shadow-inner">
              <div className="text-center">
                ...
                <p className="text-sm font-mono font-black tracking-tighter">1.{Math.floor(Math.random()*9)}K</p>
              </div>
              <div className="w-[1px] h-10 bg-white/10" />
              <div className="text-center">
                <p className="text-[9px] text-gray-500 font-black tracking-widest uppercase mb-2 opacity-50">Leads</p>
                <p className="text-sm font-mono font-black text-gold tracking-tighter">{Math.floor(Math.random()*50)+10}</p>
              </div>
