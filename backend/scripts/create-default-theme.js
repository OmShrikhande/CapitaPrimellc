...
async function createDefaultTheme() {
  try {
    ...
    const existingThemes = await db.collection('themes')
      .where('name', '==', 'default')
      .limit(1)
      .get();
    ...
    const docRef = await db.collection('themes').add(defaultTheme);

    console.log('✅ Default theme created successfully!');
    console.log('📄 Document ID:', docRef.id);
    console.log('🎨 Theme data:', defaultTheme);
    console.log('📍 Collection: themes');

  } catch (error) {
    console.error('❌ Error creating default theme:', error.message);
  } finally {
    process.exit(0);
  }
}
